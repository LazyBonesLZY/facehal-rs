...
mod proxy;
// The upstream Cargo profile hides these wrappers because it is primarily
// shipped as an NDK client library.  This standalone vendor service still
// uses only the same libbinder_ndk C APIs for registration and its thread
// pool, so keep the wrappers available in android_ndk builds as well.
#[cfg(not(trusty))]
mod service;
#[cfg(not(trusty))]
mod state;
