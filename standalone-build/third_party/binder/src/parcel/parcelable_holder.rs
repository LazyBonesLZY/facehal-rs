...
impl<STABILITY: StabilityType> Parcelable for ParcelableHolder<STABILITY> {
    ...
    fn read_from_parcel(&mut self, parcel: &BorrowedParcel<'_>) -> Result<(), StatusCode> {
        ...
        if !crate::compat::has_api31() {
            // API 29/30 cannot copy an opaque nested parcel. The Face HAL does
            // not consume extension payloads, so preserve the caller's parcel
            // position and discard only that unknown extension.
            unsafe {
