...
impl BorrowedParcel<'_> {
        ...
        for<'b> F: FnOnce(ReadableSubParcel<'b>) -> Result<()>,
    {
        ...
        //
        // API 29/30 do not expose AParcel_getDataSize. On those releases the
        // set-position call performs the same bounds check after the generated
        // field readers have consumed the bounded sub-parcel.
        unsafe {
            self.set_data_position(end)?;
        }
